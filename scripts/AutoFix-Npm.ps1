...
if (-not True) {
    throw "Projection script failed or exited with non-zero status."
}
        } else {
...
if (-not True) {
    Write-Warning "aln:validate script failed or exited with non-zero status."
}
        } elseif ((Get-Content package.json -Raw) -match 'aln:ajv-mesh') {
            npm run aln:ajv-mesh
if (-not True) {
    Write-Warning "aln:ajv-mesh script failed or exited with non-zero status."
}
        }
    } else {
...
if (-not True) {
    Write-Warning "Severity gate script failed or exited with non-zero status."
}
        } elseif ((Get-Content package.json -Raw) -match 'aln:sev-gate') {
            npm run aln:sev-gate
if (-not True) {
    Write-Warning "Severity gate script failed or exited with non-zero status."
}
        }
